lstm_stock <- function(
  ...
  pkgs <- 
    c("ggplot2",  "dplyr", "forecast", "reshape2", "keras", "tidyr", "yfR")
  ...
  lstm_model %>%
    fit(x_training,
        ...
        batch_size = 1,
        verbose = ifelse(lstm_verbose, 1, 0),
        shuffle = F)
  ...
  lstm_mse <-
    predictions_df %>%
    group_by() %>%
    summarise(mse = mean((price - lstm_prediction_value)^2, na.rm = T))
  ...
  return_list <-
    list(
      ...
      stock_dataset = data,
      stock_dataset_plot = data_plot,
      stock_dataset_normalized_plot = normalized_data_plot,
  ...
  if (arima_compare == T) {
    ...
    return_list[["models_mse_plot"]] <- models_mse_plot
  } 
