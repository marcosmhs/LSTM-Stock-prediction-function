lstm_stock = function(
  ...
  # as we are working with time series, it necessary set a cut point to spare training and test data according with the percentage defined by the user, the default value will be 80%
  cut_point = 
    round(training_data_percentage * nrow(data))
  
  # --- data prep
  
  # as we observe the data we can see a trend of downing or growing and this kind of phenomenon reduce the acuracy of the model and the capacity of the reunal net make acertive predictions. To solve that problem we need to remove that trend, by using the  difference between the values and they previus value
  data_without_trend =
    data %>%
  ...
  # now the data need to be normalized in -1 until 1 scale where all values will be replaced with a proportional value on that scale
  normalized_data = 
    data_without_trend %>%
  ...
  # batch_input_shape determinates the format of how the data will move in the neural net, on this case a 3 dimentions matrix
  # stateful indicate to the model generator the state of the last sample processed and if this sample should be used on the next sample processed
  lstm_model = 
...
return_petr4 = lstm_stock(origin = '^BVSP', stock_name = 'PETR4.SA', start_date = '2015-01-01', lstm_executions = 100, arima_compare = T)
return_mglu3 = lstm_stock(origin = '^BVSP', stock_name = 'MGLU3.SA', start_date = '2015-01-01', lstm_executions = 100, arima_compare = T)
return_ntco3 = lstm_stock(origin = '^BVSP', stock_name = 'GGBR4.SA', start_date = '2015-01-01', lstm_executions = 100, arima_compare = T)
